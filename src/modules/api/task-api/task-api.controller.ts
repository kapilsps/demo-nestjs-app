...
export class TaskApiController {
    ...
    @Get(':id')
    async findOne(@Param('id', ParseIntPipe) id: number, @GetUser() user: User): Promise<Task>{
        return await this.taskApiService.findOne(id, user);
    }

    @Post('/create')
    async create(@Body(ValidationPipe) body:CreateTaskDto, @GetUser() user: User): Promise<Task>{
        return await this.taskApiService.create(body, user);
    }

    @Patch('/update/:id')
    async update(@Param('id', ParseIntPipe) id: number, @Body(ValidationPipe) body: UpdateTaskDto, @GetUser() user:User): Promise<Task>{
        return await this.taskApiService.update(id, body, user);
    }

    @Delete('/delete/:id')
    async delete(@Param('id', ParseIntPipe) id: number, @GetUser() user: User): Promise<void | {message:string}>{
