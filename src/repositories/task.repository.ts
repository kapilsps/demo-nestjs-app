...
export class TaskRepository extends Repository<Task>{
    ...
    async getAllTask(user: User): Promise<Task[]>{
        return await this.createQueryBuilder('tasks').where('userId = :userId', {
            userId: user.id
        }).orderBy('createdAt', 'DESC').getMany();
    }
    
    async getTask(id: number, user: User):Promise<Task> {
        const task = await this.createQueryBuilder('tasks').where('tasks.id = :id and tasks.userId = :userId', {id, userId:user.id}).getOneOrFail();
    ...
    async deleteTask(id:number, user:User):Promise<void|{message:string}>{
